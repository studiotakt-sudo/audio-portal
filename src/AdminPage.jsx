...
function WaveformBg({ peaks, progress, duration, accentColor, baseColor }) {
  ...
  useEffect(() => {
    ...
    peaks.forEach((peak, i) => {
      ...
      ctx.globalAlpha = isPlayed ? (0.4 + peak * 0.4) : (0.15 + peak * 0.2)
  ...
  const handleFileDrop = (e) => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        const decoded = await audioCtx.decodeAudioData(ev.target.result)
        audioCtx.close()
        ...
        setPendingFile({ file, duration: decoded.duration, waveformPeaks: normalized })
      } catch(e) {
        // Fallback — just get duration via Audio element
        const url = URL.createObjectURL(file)
        const audio = new Audio(url)
        audio.addEventListener('loadedmetadata', () => { URL.revokeObjectURL(url); setPendingFile({ file, duration: audio.duration, waveformPeaks: [] }) })
        audio.addEventListener('error', () => { URL.revokeObjectURL(url); setPendingFile({ file, duration: null, waveformPeaks: [] }) })
      }
    }
