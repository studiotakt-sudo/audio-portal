...
function TrackManager({ tracks, clients, onRefresh, onPlay, currentTrack, onToast }) {
  ...
  const handleFileDrop = (e) => {
    ...
    audio.addEventListener('loadedmetadata', () => { URL.revokeObjectURL(url); setPendingFile({ file, duration: audio.duration }) })
    audio.addEventListener('error', () => { URL.revokeObjectURL(url); setPendingFile({ file, duration: null }) })
        ...
        <>
          ...
          <div className="track-list" style={{paddingBottom:80}}>
            {filtered.map((track, i) => {
              ...
              return (
                <div key={track.id}
                  draggable={!search && !isEditing}
                  onDragStart={() => handleDragStart(i)}
                  onDragEnter={() => handleDragEnter(i)}
                  onDragEnd={handleDragEnd}
                  onDragOver={e => e.preventDefault()}
                >
                  <div className={`track-row ${currentTrack?.id===track.id&&currentTrack?.versionIdx===undefined?'playing':''} ${isEditing?'editing':''} ${isFeatured?'featured-row':''}`}
                    onClick={() => !isEditing && onPlay(track)}
                    style={isFeatured ? {borderColor: T.amber, borderLeftWidth:3} : {}}>
                    {/* Drag handle */}
                    {!search && (
                      <div style={{position:'absolute', left:0, top:0, bottom:0, width:12, display:'flex', alignItems:'center', justifyContent:'center', cursor:'grab', color:T.textMuted, fontSize:10, opacity:0.4}}
