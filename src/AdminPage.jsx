...
function TrackManager({ tracks, clients, onRefresh, onPlay, currentTrack, onToast }) {
  ...
  // For adding a version inside the edit panel
  ...
  const uploadTrack = async () => {
    ...
    const { error: uploadError } = await supabase.storage
      .from('audio-tracks').upload(filePath, pendingFile, { contentType: pendingFile.type })
  ...
  const removeVersion = (idx) => {
    setEditState(s => ({ ...s, versions: s.versions.filter((_, i) => i !== idx) }))
  }
                          ...
                          {editState.versions.length > 0 && (
                            <div style={{display:'flex', flexDirection:'column', gap:6, marginBottom:12}}>
                              {editState.versions.map((v, vi) => (
                                <div key={vi} style={{display:'flex', alignItems:'center', gap:10, background:'#0e1018', padding:'8px 12px', borderRadius:3, border:`1px solid ${T.border}`}}>
                                  <span style={{fontFamily:'Space Mono,monospace', fontSize:11, color:T.amber, flex:1}}>{v.label}</span>
                                  <span style={{fontFamily:'Space Mono,monospace', fontSize:10, color:T.textMuted}}>{v.file_name}</span>
                                  <button className="btn-icon" style={{width:24, height:24, color:T.red, borderColor:'#3a1010', fontSize:12}}
