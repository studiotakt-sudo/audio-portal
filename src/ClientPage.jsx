...
export default function ClientPage({ clientRow, onPlay, playerProps, onToast, registerNextResolver }) {
  ...
  const fetchTracks = async () => {
    ...
    const TRACK_COLS = 'id, title, file_name, file_path, file_size, mime_type, tags, uploaded_at, versions, duration, sort_order, featured, waveform_peaks, featured_image, bpm, composer_id, is_published, hidden'
    ...
    // to select('*') so the client still sees their tracks, and strip the two
    // admin-only fields client-side so they're never displayed.
    if (error) {
      console.error('track fetch (explicit cols) failed, falling back:', error.message)
      const fb = await supabase.from('tracks').select('*').order('sort_order', { ascending: true })
      data = (fb.data || []).map(({ admin_notes, project_file_ref, ...rest }) => rest)
    ...
    const mine = (data || []).filter(t =>
      ...
      t.hidden !== true             // never show hidden (e.g. exclusively-pitched) tracks
    )
    setTracks(mine)
    setLoading(false)
