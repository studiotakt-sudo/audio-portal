...
const TrackRowBase = memo(function TrackRowBase({
  ...
  accentColor, mutedColor, progress, duration,
...
export default function ClientPage({ clientRow, onPlay, playerProps, onToast }) {
  ...
  const TrackRow = ({ track, i }) => {
    ...
    // Active version index (if a version of this track is playing), else null.
    ...
    return (
      <TrackRowBase
        ...
        onDownload={download}
        /* progress/duration only flow to a row that's actually playing, so
           inactive rows receive constant 0 and never re-render on a tick */
